...
const driverProto = {
  ...
  selectSql: function selectSql(opts) {
    ...
    function withJoin() {
      ...
      forEach(relationships, function (rel, key) {
        ...
        joinString += this.joinSql({
          primary: { name: table, key: joinKey },
          secondary: { name: otherTable, key: rel.foreign },
    ...
    function getFields(name, tables) {
  ...
  joinSql: function (opts) {
    const primary = opts.primary // { name, column }
    const secondary = opts.secondary // { name, column }
    const joinType = opts.optional ? ' LEFT ' : ' INNER '
    const primaryKey = fmt('%s.%s', primary.name, primary.key)
    const secondaryKey = fmt('%s.%s', secondary.name, secondary.key)

    const statement = joinType
      + ' JOIN ' + this.escapeId(secondary.name)
