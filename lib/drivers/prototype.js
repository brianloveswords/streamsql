...
module.exports = {
  ...
  whereSql: function whereSql(table, conditions, relationships) {
    ...
    function generateSql (conditions) {
      ...
      var clauses = map(conditions, generateClauses)

      for (var key in relationships) {
        var rel = relationships[key]
        if (rel.foreign && rel.conditions) {
          table = table + JOIN_SEP + rel.foreign.as
          for (var k in rel.foreign.conditions) {
            conditions[k] = rel.foreign.conditions[k]
          }
          var test = map(rel.foreign.conditions, generateClauses)
          clauses = clauses.concat( test )
        }
      }
      return clauses.join(' AND ')
    }

    function generateClauses (val, key) {
  ...
  selectSql: function selectSql(opts) {
    ...
    function withJoin() {
      ...
      const clauses = [
        joinClause,
        this.whereSql(table, conds, relationships),
        this.orderSql(table, opts.order),
        this.limitSql(opts.limit, opts.page),
      ]
  ...
  fixHasMany: function (globalRow, opts, callback) {
    ...
    function fetchVia (rel, callback) {
      ...
      options.relationships[rel.foreign.table] = {
        ...
        foreign: rel.foreign,
        conditions: rel.foreign.conditions,
